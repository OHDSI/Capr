...
### Inspecting a cohort definition

Capr provides `show()` and `summary()` methods so a cohort can be inspected in
the console without dumping the raw S4 slots. Typing an object (or calling
`show()`) gives a compact, one-block overview; `summary()` expands the
structure, including nested correlated criteria.
...
Passing `listConceptSets = TRUE` appends a concept-set section to the bottom of
the summary showing each set's id and per-concept `descendants`/`excluded`/
`mapped` logic. If the cohort has been hydrated with `getConceptSetDetails()`
(see the *Capr-conceptSets* vignette), that section also shows each concept's
name and domain.
