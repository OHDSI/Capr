...
To build all these cohorts using a template, we can easily define the logic in Capr. Capr templates are functions where the input is a concept set that varies the logic.
...
To save multiple cohorts from a list, apply `writeCohort` across the list using `purrr::walk`. 
